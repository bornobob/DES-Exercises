...
def create_runner():
	...
	mission_Mission = Mission([BorderAction(priority=5), ColorDetAction(colors=[ColorSensor.COLOR_RED], priority=4), DontDrownAction(lakes=[ColorSensor.COLOR_RED, ColorSensor.COLOR_YELLOW, ColorSensor.COLOR_BLUE], priority=3), UltrasoundAction(rotate_degrees=0.5, dodge_rocks=False, priority=2), DriveAction(priority=1)], SpeakCelebration(''))
